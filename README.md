...
- **Пул WG/AWG-конфигов — выдача на пользователя/устройство.** Один WG-ключ нельзя
  ...
  устройство» (один конфиг на пару пользователь+HWID). Плюс ручная привязка
  конкретного конфига к пользователю/устройству в обход сквадов, «липкое»
  закрепление, авто-возврат слота после простоя и расчёт потребности — по
  фактическим устройствам из базы HWID, которую прослойка ведёт по hwid-событиям
  вебхука панели. Вкладка доп-конфигов разделена на «простые» (VLESS/общие) и
