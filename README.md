...
- **Грейс-сквад для истёкших** — по вебхуку `user.expired` юзер не просто
  ...
  Опционально на время грейса юзеру выдаётся **внешний сквад** (его `announce` и
  заголовки из Subscription Settings панели) — исходный внешний сквад запоминается
  и возвращается вместе с остальным. По умолчанию выключено. Если внешний сквад
  не используется, тот же анонс можно отдать самой прослойкой (фолбэк): заголовок
  `announce` на время грейса, переносы строк через Enter (как в панели — реальные
  переводы строк в base64, работают во всех клиентах), кириллица — авто-base64.
- **HWID-блокировка устройств** — точечная блокировка конкретного устройства по
  ...
  админки.
- **HWID из User-Agent.** Для клиентов, которые не умеют слать HTTP-заголовки, но
  позволяют менять User-Agent (v2rayNG, Clash): прослойка извлекает device-заголовки
  (`x-hwid`, `x-device-os`, `x-ver-os`, `x-device-model`) из строки вида
  `...; x-hwid=значение; ...)` и форвардит их на панель, если настоящего заголовка в
  запросе нет. Реальный заголовок имеет приоритет. Опция выключена по умолчанию,
  набор ключей настраивается во вкладке «Подключение». User-Agent задаётся
  пользователем, поэтому `x-hwid` так можно подменить — HWID остаётся удобством, а
  не защитой.
...
- **Проброс доп. конфигов по сквадам (бета).** В выдачу подписки можно добавлять
  ...
  `vless://` и отдаются как base64/JSON или Clash. Инжект в xray-json (клиенты на
  сыром xray-конфиге, например Happ) включается отдельным тумблером; туда идут
  обычный WireGuard (через userspace-gVisor, `noKernelTun` — поднимается и на
  мобиле) и VLESS, а AmneziaWG в xray-json не отдаётся (xray не умеет
  amnezia-обфускацию). **Функция в бета-тесте:** поведение в разных клиентах ещё
  обкатывается — включайте и проверяйте на тестовом пользователе.
- **Пул WG/AWG-конфигов — выдача на пользователя/устройство.** Один WG-ключ нельзя
  делить между устройствами (у пира один endpoint, при двух устройствах на одном
  ключе соединения флапают), поэтому прослойка не генерирует ключи, а раздаёт
  готовые `.conf` из загруженного пула, закрепляя за подписчиком отдельный, никем
  больше не используемый конфиг. Режим выбирается на каждый сквад: «общий» (всем,
  как доп-конфиги выше), «на пользователя» (один конфиг на подписчика) или «на
  устройство» (один конфиг на пару пользователь+HWID). В режимах «на
  пользователя/устройство» слот выдаётся отдельно по каждому типу — устройство
  получает по одному конфигу каждого типа (WireGuard и AmneziaWG), а клиент берёт
  тот, что умеет. Плюс ручная привязка конкретного конфига к пользователю/устройству
  в обход сквадов, «липкое» закрепление, авто-возврат слота после простоя и расчёт
  потребности — по фактическим устройствам из базы HWID, которую прослойка ведёт по
  hwid-событиям вебхука панели. Пакетная загрузка конфигов (файлами или вставкой
  текста, до 200 файлов за раз) и массовые операции в списке — выделить все, удалить
  выбранные или все. Вкладка доп-конфигов разделена на «простые» (VLESS/общие) и
  «WG / AWG» (пул). Функция в бета-тесте.
...
- **Веб-админка** поверх API панели: список пользователей со ссылками подписки
  ...
  пересылки, грейс-юзеров), брендинг. Настройки на вкладках сохраняются
  автоматически, без кнопки «Сохранить»: переключатели и выпадающие списки —
  сразу по изменению, текстовые поля — при потере фокуса, без перезагрузки
  страницы. Секретные поля (токены, пароли) по-прежнему сохраняются по явной
  кнопке.
...
- **Доп. конфиги по сквадам (бета).** Подмешать в подписку дополнительный
  ...
  клиент. WG/AWG можно раздавать из пула — отдельный конфиг на подписчика или
  устройство (один ключ не делится между устройствами), с ручной привязкой.
  Функция пока в бета-тесте.
...
- **Грейс-сквад** — по умолчанию **ВЫКЛЮЧЕН** (`grace_squad_enabled = 0`). Если
  ...
  у пользователя статус, сквады (внутренние и, опционально, внешний), лимит трафика,
  стратегию сброса, лимит HWID и дату истечения; при оплате/окончании грейса —
  возвращает обратно. **Это самый
